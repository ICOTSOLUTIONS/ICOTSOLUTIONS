<footer>
    <div class="container">
        <div class="row">
            <div class="col-md-3 col-lg-3 col-xl-3">
                <div class="footer-widget">
                    <div class="footer-logo">
                        <a href="#"><img src="assets/img/logo/icotsolutions_web.svg" alt=""></a>
                    </div>
                    <address>
                        <h4>Office</h4>
                        <p>PAKISTAN.</p>
                    </address>
                    <ul class="social-media-icons">
                        <li><a href="https://www.facebook.com/icotsolutions/" target="_blank"><i class="fab fa-facebook-f"></i></a></li>
                        <li><a href="https://www.instagram.com/icotsolutions/?hl=en" target="_blank"><i class="fab fa-instagram"></i></a></li>
                        <li><a href="https://www.linkedin.com/in/icot-solutions-a5642a237/" target="_blank"><i class="fab fa-linkedin-in"></i></a></li>
                        <li><a href="https://twitter.com/IcotSolutions" target="_blank"><i class="fab fa-twitter"></i></a></li>
                        <li><a href="https://github.com/ICOTSOLUTIONS" target="_blank"><i class="fab fa-github"></i></a></li>
                        <li class="mt-2"><a href="https://www.youtube.com/channel/UCtvL2HcghShfPJBYsc3w-zQ" target="_blank"><i class="fab fa-youtube"></i></a></li>
                        <li class="mt-2"><a href="https://discord.com/login?redirect_to=%2Flogin" target="_blank"><i class="fab fa-discord"></i></a></li>
                    </ul>
                </div>
            </div>
            <div class="col-md-3 col-lg-3 col-xl-3">
                <div class="footer-widget">
                    <h4>Our Services</h4>
                    <ul class="footer-menu">
                        <li><a href="{{url('/webDesigning')}}">Web Designing</a></li>
                        <li><a href="{{url('/WebDevelopment')}}">Web Development</a></li>
                        <li><a href="{{url('/AppDevelopment')}}">App Development</a></li>
                        <li><a href="{{url('/GraphicDesigning')}}">Graphic Designing </a></li>
                        <li><a href="{{url('/DigitalMarketing')}}">Digital Merketing</a></li>
                        <!-- <li><a href="{{url('/CohesiveBranding')}}">Cohesive Branding Solutions</a></li> -->
                    </ul>
                </div>
            </div>
            <div class="col-md-3 col-lg-3 col-xl-3">
                <div class="footer-widget">
                    <h4>Our Comapny</h4>
                    <ul class="footer-menu">
                        <li><a href="{{url('/')}}">Home</a></li>
                        <li><a href="{{url('/about')}}">About Us</a></li>
                        <li><a href="{{url('/service')}}">Services</a></li>
                        <li><a href="{{url('/portfolio')}}">Portfolio</a></li>
                        {{-- <li><a href="blog.html">Blog</a></li>
                        <li><a href="contact.html">Career</a></li>
                        <li><a href="services.html">Pricing Plan</a></li> --}}
                    </ul>
                </div>
            </div>
            <div class="col-md-3 col-lg-3 col-xl-3">
                <div class="footer-widget">
                    <h4>Contacts</h4>
                    <div class="number">
                        <div class="num-icon">
                            <i class="fas fa-phone-alt"></i>
                        </div>
                        <div class="phone">
                            <a href="tel:+923305069722">+92 330 5069 722</a>
                        </div>
                    </div>
                    <!--<div class="number">-->
                    <!--    <div class="num-icon">-->
                    <!--        <i class="fas fa-phone-alt"></i>-->
                    <!--    </div>-->
                    <!--    <div class="phone">-->
                    <!--        <a href="tel:03360394622">+92 336 0394 622</a>-->

                    <!--    </div>-->
                    <!--</div>-->
                    <div class="office-mail">
                        <div class="mail-icon">
                            <i class="far fa-envelope"></i>
                        </div>
                        <div class="email">
                            <a href="mailto:info@icotsolutions.com" target="_blank"><span class="__cf_email__" data-cfemail="1e777078715e7b667f736e727b307d7173">info@icotsolutions.com</span></a>
                            <!-- <a href="tell:info@support.com"><span class="__cf_email__"
                                    data-cfemail="4b22252d240b383e3b3b24393f65282426">info@icotsolutions.com</span></a> -->
                        </div>
                    </div>
                    <div class="address">
                        <div class="address-icon">
                            <i class="fas fa-map-marker-alt"></i>
                        </div>
                        <!--<p>UAE & PAKISTAN.</p>-->
                        <p>PAKISTAN.</p>
                    </div>
                </div>
            </div>
        </div>
        <div class="footer-bottom">
            <div class="row align-items-center">
                <div class="col-12 col-md-4 col-lg-4 col-xl-5">
                    <div class="copy-txt">
                        <span><b>ICOTSOLUTIONS</b></span>
                    </div>
                </div>
                <div class="col-12 col-md-8 col-lg-8 col-xl-7">
                    <ul class="footer-bottom-menu">
                        <li><a href="#">Privacy Policy</a></li>
                        <li><a href="#">Terms of Use</a></li>
                        <li><a href="#">Support Policy</a></li>
                        <li><a href="#">Terms of Service</a></li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
</footer>